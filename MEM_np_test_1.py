...
outputDirectory = 'Test_1core_Beau/'
...
t_int = 1
#Dictionaries
def PlatDepth(arra, ysr, year):
    x = 0
    arrayYear = np.zeros(shape=(y_pixels,x_pixels))
    for lin in arra:
        arrayYear[x] = readFn(ysr, lin)
        x = x + 1
    #Storing the output array in the total array
    totalArray[year-1] = arrayYear
...
totalArray = np.empty((T,y_pixels,x_pixels))
...
for lin in ras.read(1):
    Array[x] = readFn(ySLR[1], lin)
    x = x + 1

#Storing the result of first year('Array') into the totalArray
totalArray[0] = Array

#Running all the years by passing the output array of the previous year as a paramter
for i in range(2, T+t_int, t_int):
    PlatDepth(totalArray[i-2], ySLR[i], i)
#Writing the files from total array
for i in range(1, T+t_int, t_int):
    name_i = str(i)
    outputFileName = outputDirectory + '//' + 'year_' + name_i + '.tif'
    array_to_raster(totalArray[i-1], outputFileName)
